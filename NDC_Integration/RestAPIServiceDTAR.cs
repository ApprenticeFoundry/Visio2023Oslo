...
namespace Visio2023Foundry.Services;
...
public interface IRestAPIServiceDTAR
{
    ...
    Task<UDTO_File> PDFSnapshot(DT_Base64 file);
}

public class RestAPIServiceDTAR : IRestAPIServiceDTAR
{
    private string ServerURL { get; set; } = "https://rondtar.azurewebsites.net/"; //"https://localhost:5001";  //; "https://localhost:5001";  // 
    ...
    public async Task<List<string>> GetVersion()
    {
        try
        {
            ...
            var wrapper = HydrateWrapper<string>(text, true);
    ...
    public async Task<List<DT_MILDocument>> GetRootDocument()
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<DT_MILDocument>(text, true);
    ...
    public async Task<List<DT_World3D>> GetWorlds()
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<DT_World3D>(text, true);
            var coll = result!.payload;
            return coll.ToList();
        }
        catch (Exception ex)
    ...
    public async Task<List<DT_ProcessPlan>> GetAllProcessPlans()
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<DT_ProcessPlan>(text, true);
            var coll = result!.payload;
            return coll.ToList();
        }
        catch (Exception ex)
    ...
    public async Task<DT_ComponentReference> GetComponentForPlan(string guid)
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<DT_ComponentReference>(text, true);
            var coll = result!.payload;
            return coll.First();
        }
        catch (Exception ex)
    ...
    public async Task<List<VersionInfo>> ModuleWorkbook(string guid)
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<VersionInfo>(text, true);
            var coll = result!.payload;
            return coll.ToList();
        }
        catch (Exception ex)
    ...
    public async Task<List<AppStoreDefinition>> AppStoreRunOffline(string guid, bool toggle)
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<AppStoreDefinition>(text, true);
            var coll = result!.payload;
            return coll.ToList();
        }
        catch (Exception ex)
    ...
    public async Task<List<UserMode>> SetApplicationMode(UserMode mode)
    {
        try
        {
            ...
            var c = new StringContent(s, Encoding.UTF8);

            var response = await client.PostAsync(path, c);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<UserMode>(text, true);
            var coll = result!.payload;
            return coll.ToList();
        }
        catch (Exception ex)
    ...
    public async Task<List<DT_AssetFile>> GetAssetFiles()
    {
        try
        {
            ...
            var response = await client.GetAsync(path);
            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<DT_AssetFile>(text, true);
            var coll = result!.payload;
            return coll.ToList();
        }
        catch (Exception ex)
    ...
    public async Task<List<UDTO_File>> UploadFile(IBrowserFile file)
    {
        try
        {
            ...
            var text = await response.Content.ReadAsStringAsync();
            var success = HydrateWrapper<UDTO_File>(text, true);
    ...
    public async Task<UDTO_File> PDFSnapshot(DT_Base64 data)
    {
        try
        {
            var path = new Uri($"{APIPath}/PDF/FoundryCanvas");
            var client = new HttpClient();

            string json = StorageHelpers.Dehydrate<DT_Base64>(data, true);
            var httpContent = new StringContent(json, System.Text.Encoding.UTF8, "application/json");

            var response = await client.PostAsync(path, httpContent);

            var text = await response.Content.ReadAsStringAsync();
            var result = HydrateWrapper<UDTO_File>(text, true);
            return result.payload.ToList().First();
        }
        catch (Exception ex)
        {
            this.note = ex.Message;
